...
export function PaymentsSection() {
  ...
  useEffect(() => {
    fetch("/api/payment-methods", { credentials: "include" })
      .then((r) => r.json())
      .then((d) => {
        const raw = Array.isArray(d) ? d : d?.paymentMethods ?? d?.payment_methods ?? d ?? [];
  ...
  const openEdit = (m: PaymentMethod) => {
    ...
    setForm({
      ...
      label_fr: m.labelFr ?? (m as Record<string, string>).label_fr ?? "",
      label_ar: m.labelAr ?? (m as Record<string, string>).label_ar ?? "",
  ...
  return (
    <div className="space-y-5 max-w-4xl">
      <div className="rounded-xl border bg-card overflow-hidden">
        <div className="px-5 py-4 border-b">
          ...
          <p className="text-xs text-muted-foreground">
            Manage payment methods available for billing. Labels can be customized per language.
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                <th className="px-5 py-3 text-start font-medium text-muted-foreground">Type</th>
                <th className="px-5 py-3 text-start font-medium text-muted-foreground">Label</th>
                <th className="px-5 py-3 text-center font-medium text-muted-foreground">Status</th>
                <th className="px-5 py-3 text-end font-medium text-muted-foreground">Order</th>
            ...
            <tbody>
              {methods.map((m) => {
                ...
                return (
                  ...
                  >
                    <td className="px-5 py-3 font-medium">{formatType(m.type)}</td>
      ...
      <SlideOver open={slideOpen} onClose={closeSlide} title="Edit Payment Method">
